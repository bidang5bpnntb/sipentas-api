...
function handleLogin(e) {
  ...
  const savedPass = localStorage.getItem('sipentas-password') || 'admin123';
...
function openDossier(id) {
  ...
  if (finalPdfName) {
    iframe.src = `${API_URL}/uploads/${encodeURIComponent(finalPdfName)}`;
  } else {
...
async function submitForm(e) {
  ...
  formData.append('tahapan', JSON.stringify(Array.from({ length: 6 }, () => ({ date: '', note: '', file: null, fileName: '' }))));
...
async function saveDokumenTambahan(e) {
  e.preventDefault();
  ...
  // --- C. KIRIM KE SERVER (PERSIST KE DATABASE) ---
  const formDataUpload = new FormData();
  formDataUpload.append('dokumenLain', JSON.stringify(c.dokumenLain));
  formDataUpload.append('auditTrail', JSON.stringify(c.auditTrail));
  if (fileInput.files.length > 0) formDataUpload.append('file', fileInput.files[0]);

  try {
    await fetch(`${API_URL}/kasus/${id}`, { method: 'PUT', body: formDataUpload });
  } catch (err) {
    console.warn('Server tidak tersedia, dokumen tambahan tersimpan lokal saja:', err);
  }

  // --- D. SIMULASI JARINGAN & UPDATE UI INSTAN ---
  setTimeout(() => {
...
async function saveTahapanUpdate(e, caseId, stepIndex) {
  ...
  // Simpan data tahapan
  if (!c.tahapan) c.tahapan = Array.from({ length: 6 }, () => ({ date: '', note: '', fileName: '' }));
